/* Average driving speed (i.e. coordinate change divided by time elapsed) of all jobs, in an hour */
...
SELECT AVG(dist/(DATE_PART('day',travel_time)*24 + DATE_PART('hour',travel_time) + DATE_PART('minutes',travel_time)/60))
FROM leg_stats