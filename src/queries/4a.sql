/*Median driving speed of each truck driver in all jobs handled by him / her 
  ...
  10 best performers in a day (highest average driving speed) */
...
SELECT driver_id, AVG(dist/(DATE_PART('day',travel_time)*24 + DATE_PART('hour',travel_time) + DATE_PART('minutes',travel_time)/60)) as speed
FROM leg_stats
GROUP BY driver_id
ORDER BY speed DESC
LIMIT 10
