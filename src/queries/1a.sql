/* Number of delivery jobs per day, peak delivery hours throughout a day, Past-7-day trend */
...
FROM legs
WHERE date_trunc('day',arrive_time) >= CURRENT_DATE - 7
GROUP BY sdate, hour
ORDER BY num_legs DESC
LIMIT 5;

/*number of jobs in past 7 days*/
SELECT DATE_TRUNC('day', start_time) as sdate, COUNT(1) as jobs_per_day
FROM jobs
WHERE sdate >= CURRENT_DATE - 7
GROUP BY sdate
ORDER BY sdate;